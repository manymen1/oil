...
@pytest.fixture
def config(tmp_path):
    data = yaml.safe_load(Path("configs/observe.yaml").read_text())
...
def test_boundaries_and_environment(config, monkeypatch):
    ...
    assert "BROKER_PRIVATE_KEY" not in restricted_environment()
...
def test_market_fixtures_cannot_claim_realtime(tmp_path):
    data = json.loads(Path("tests/fixtures/market.json").read_text())
